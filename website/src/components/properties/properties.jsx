...
const PropertiesPage = () => {
  ...
  const prevImage = (propertyId, e) => {
    ...
    setCurrentImageIndex((prev) => ({
      ...prev,
      [propertyId]:
        (prev[propertyId] - 1 + property.images.length) %
        ...
        <div className="max-w-md mx-auto px-4">
          ...
          >
            ...
            <p className="text-base text-gray-300 mb-6">
              With over 15 years of excellence, we've helped thousands find
              their dream homes
            </p>
        ...
        {filteredProperties.length === 0 ? (
          ...
          >
            {filteredProperties.map((property) => (
              ...
              >
<div
  className={`relative ${viewMode === "list" ? "sm:w-40 sm:h-auto" : "h-52"}`}
>
  <div
    className="absolute inset-0"
    style={{
      backgroundImage: `linear-gradient(rgba(0,0,0,0.2), rgba(0,0,0,0.2)), url(${property.images[currentImageIndex[property.id]]})`,
                  ...
                  {property.images.length > 1 && (
                    <>
                      <button
                        ...
                        className="absolute left-2 top-1/2 transform -translate-y-1/2 p-1 rounded-full bg-black/30 text-white hover:bg-black/50"
                      >
                        <ChevronLeft size={20} />
                      </button>
                      <button
                        onClick={(e) => nextImage(property.id, e)}
                        className="absolute right-2 top-1/2 transform -translate-y-1/2 p-1 rounded-full bg-black/30 text-white hover:bg-black/50"
                      >
                        ...
                        {property.images.map((_, index) => (
                          <div
                            ...
                            className={`w-2 h-2 rounded-full ${
                              currentImageIndex[property.id] === index
                                ? "bg-white"
                                : "bg-white/50"
                            }`}
                          />
                      ...
                      >
                        <div className="mt-4 pt-3 border-t border-gray-100">
                          <p className="text-sm text-gray-600 mb-3">
                            This stunning {property.type.toLowerCase()} features{" "}
                            {property.beds} bedrooms and {property.baths}{" "}
                            bathrooms spread across{" "}
                            {property.sqm.toLocaleString()} square meters of
                    ...
                    {["All", "House", "Apartment", "Villa"].map((type) => (
                      <button
                        ...
                        className={`py-2 px-3 text-sm rounded-lg border ${
                          ...
                          (filters.propertyType === "all" && type === "All")
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {["Any", "1+", "2+", "3+", "4+"].map((option) => (
                      <button
                        key={option}
                        className={`py-2 px-3 text-sm rounded-lg border ${
                          filters.beds === option.toLowerCase() ||
                          (filters.beds === "any" && option === "Any")
                            ? "border-red-500 bg-red-50 text-red-600"
                            : "border-gray-200 bg-white text-gray-700"
                        }`}
                        onClick={() =>
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {["Any", "1+", "2+", "3+", "4+"].map((option) => (
                      <button
                        key={option}
                        className={`py-2 px-3 text-sm rounded-lg border ${
                          filters.baths === option.toLowerCase() ||
                          (filters.baths === "any" && option === "Any")
                            ? "border-red-500 bg-red-50 text-red-600"
                            : "border-gray-200 bg-white text-gray-700"
                        }`}
                        onClick={() =>
                ...
                <nav>
                  <ul className="space-y-1">
                    {[
                      ...
                      <li key={index}>
                        <button
                          className="flex items-center gap-3 w-full px-4 py-3 rounded-lg hover:bg-gray-50 text-gray-700"
